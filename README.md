...
- 🎛️ **播放键 / 音量条 / 进度条按 macOS Music 重做**（两处页面同步）：播放暂停是实心三角与双竖条（不是 DSH 的圆圈套三角）；滑杆是细轨 + 中性色填充，圆钮是 macOS 的**白色胶囊（椭圆）钮**（进度条上悬停/拖拽才现，音量条上常显）；右侧时间统一显示 **-剩余**；全屏播放器的进度条与封面**同宽**且时间换行到条下方，底部条进度条铺满中间列、时间仍内联；进度条拖动由**自绘命中区**接管（`pointer-events:none` 掉滑杆本体）：单击即快进到该点、拖动期间只画界面不碰音频、**松手才 seek 一次**（流式文件被反复 seek 会不停中断 Range 请求，快速拖动会卡死）
- ✨ **动效与材质取自 Apple Music 前端**（[music.apple.com/cn/new](https://music.apple.com/cn/new) 的 CSS）：缓动曲线用他们那三条（`cubic-bezier(.215,.61,.355,1)` easeOutCubic、`cubic-bezier(.23,1,.32,1)` easeOutQuint、`cubic-bezier(.76,.665,.37,1.35)` 带过冲）；全屏播放器出场 fade+scale、收起播 200ms 退场动画（对应他们的 `modalZoomIn/modalZoomOut`）；换曲时封面淡入 + 轻微缩放（`crossfade` + `artwork-scroll-effects` 的克制版）；按钮按压 `scale(.92)`（他们的按下就是 .88~.92）；毛玻璃用他们的配方 `backdrop-filter: saturate(180%) blur(60px)`（大面板 blur 48px）。全部包在 `prefers-reduced-motion` 里
- 🎬 **MV（音乐视频）**：库里混有 mp4 / m4v / mov / webm / mkv / avi / ts / wmv / flv 时照常列出，播放视频轨会在全屏播放器里**出画面**（列表/播放条上有 `MV` 标识）。判定分三档（参考 Jellyfin 的 Direct Play / Direct Stream / Transcode）：**direct** 直接播、**remux** 用 ffmpeg 换壳、**transcode** 重编码；AVI/TS 这类 Chromium 解不开的容器，以及该构建没有解码器的编码（HEVC 等）自动落到后两档，产物缓存在临时目录、下次秒开（缓存键含路径+大小+mtime，文件一改就失效）。**没有 ffmpeg 也能用**：直出的格式照常播，需要换壳的会给出安装提示
- 🎵 **正在播放指示**（Apple Music 的 `.playing-bars`）：正在播放那一行的封面上叠三根跳动条，鼠标悬停时跳动条淡出、换成播放/暂停图标（对应他们的 `--playButtonOpacity` + `.playing-bars-hover`），列表与全屏播放器的队列两处都有
- 🖱️ **另外两处跟着 Apple Music 的规格**：列表行圆角用他们的 `--songs-list-row-border-radius:12px`；滚动条做成 macOS 悬浮样式（平时隐形、悬停/滚动浮出，他们直接 `display:none`）；键盘焦点环用他们的做法 `box-shadow: 0 0 0 3px rgba(keyColor,.6)`
...
lib/tagwriter.js   标签写入 worker 线程入口（node-taglib-sharp）
                   （MV 的换壳/转码走 ffmpeg：探测顺序 DSH_MUSIC_FFMPEG → Homebrew/MacPorts → PATH）
```
