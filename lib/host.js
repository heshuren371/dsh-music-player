...
async function matchTrack(track, queries) {
  ...
  for (const query of queries) {
    const results = await Promise.allSettled(MATCH_SOURCES.map((source) => searchSource(source, query)));
    results.forEach((result, index) => {
      if (result.status === 'rejected') {
        errors.push(MATCH_SOURCES[index].name + ': ' + (result.reason?.message ?? String(result.reason)));
      } else {
        for (const candidate of result.value) mergeCandidate(merged, candidate);
      }
    });
    const best = rankCandidates(track, merged)[0];
...
/**
 * 把匹配到的元数据写回音频文件（ID3 / Vorbis / MP4 / WAV…）。封面是尽力而为：
 * 标签库不支持某个格式时只报告失败，绝不中断后续的重命名与列表更新。
 */
...
function createHost(ctx) {
  ...
  async function handle(req, res) {
    ...
    if (pathname === '/dsh-music/api/match' && req.method === 'GET') {
      ...
      const result = await matchTrack(track, queries.length > 0 ? queries : [track.name]);
      sendJson(res, 200, {
