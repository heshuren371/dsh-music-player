...
function createHost(ctx) {
  ...
  async function handle(req, res) {
    ...
    const url = new URL(req.url ?? '/', 'http://127.0.0.1');
    const isSystemArt = url.pathname === '/dsh-music/api/system-art' || url.pathname === '/api/dsh-music/system-art';
    if (!isSystemArt && isUntrustedRequest(req)) throw statusError(403, 'untrusted request rejected');
    return dispatch(req, res, isSystemArt ? 'system-art' : 'legacy');
  }
