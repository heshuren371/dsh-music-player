window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
.dshm-root{box-sizing:border-box;width:100%;height:100%;min-height:0;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-1);font:var(--dsw-font-xs-13);flex-direction:column;display:flex;overflow:hidden;padding-bottom:calc(var(--dsh-composer-height,152px) + 16px)} /* 根节点声明 data-conversation-composer-overlay 后 composer 悬浮在底部：此处预留其高度（变量由 ConversationRoot 实时发布），避免遮挡播放栏 */
...
`;
    ...
    function createPlayer() {
      ...
      const visibleRows = () => {
        ...
        if (state.sortKey === "none") return rows;
    ...
    function MusicView() {
      ...
      return h("div", { className: "dshm-root", ref: rootRef, "data-conversation-composer-overlay": "" },
        header,
        dirEditor,
        confirmBar,
    ...
    class ErrorBoundary extends React.Component {
      ...
      render() {
        if (this.state.error !== null) {
          return h("div", { className: "dshm-root", "data-conversation-composer-overlay": "" },
