...
`;
    ...
    function createPlayer() {
      ...
      const restoreLastPlayed = () => {
        ...
        if (last === undefined || typeof last.id !== "string") return;
      ...
      audio.addEventListener("timeupdate", () => {
        ...
        // Throttled-ish persistence of the play position (timeupdate fires ~4Hz;
        // localStorage writes at that rate are cheap enough at this payload size).
        const track = state.tracks[state.current];
        if (track !== undefined) savePrefs({ last: { id: track.id, time: Math.floor(audio.currentTime) } });
      });
      ...
      audio.addEventListener("error", () => {
        ...
        // Re-check the index inside the timer: the user may have picked another
        // song during the grace period, and that choice must win.
    ...
    function MusicView() {
      ...
      // Search filter: rows keep their library index so playback addressing
      // and the "#" column stay honest.
      const query = state.query.trim().toLowerCase();
      const filteredTracks = query.length === 0
        ? state.tracks.map((track, index) => ({ track, index }))
        : state.tracks.reduce((rows, track, index) => {
            const haystack = (track.title + " " + (track.artist ?? "") + " " + track.name).toLowerCase();
            if (haystack.includes(query)) rows.push({ track, index });
            return rows;
          }, []);

      // Column sorting on top of the filter; "none" keeps library order.
      const visibleTracks = state.sortKey === "none" ? filteredTracks : [...filteredTracks].sort((a, b) => {
      ...
      const sortHeader = (key, labelKey, className) => h("th", {
        className: (className ?? "") + " dshm-sortable" + (state.sortKey === key ? " dshm-sorted" : ""),
        onClick: () => player.toggleSort(key),
      }, t(labelKey), state.sortKey === key ? (state.sortDir === "asc" ? " ▲" : " ▼") : "");
      ...
      const header = h("div", { className: "dshm-header" },
        ...
        state.scanning
          ...
          : state.tracks.length > 0 && h("span", { className: "dshm-stats" },
              t("stats")(filteredTracks.length === state.tracks.length ? state.tracks.length : filteredTracks.length + "/" + state.tracks.length),
      ...
      if (state.dir === null) {
        ...
        body = h("div", { className: "dshm-tableWrap" },
          h("table", { className: "dshm-table" },
            ...
            h("tbody", null, visibleTracks.map(({ track, index }) => h("tr", {
