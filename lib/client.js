...
.dshm-playerScrim{position:absolute;inset:0;background:color-mix(in srgb, var(--dsw-alias-bg-base) 78%, transparent);-webkit-backdrop-filter:blur(30px);backdrop-filter:blur(30px);pointer-events:none}
...
.dshm-playerProgress{width:100%;min-width:0;flex-direction:column;gap:6px;display:flex}
.dshm-timeRow{justify-content:space-between;align-items:center;gap:8px;display:flex}
.dshm-playerProgress .dshm-time{width:auto;text-align:left}
.dshm-playerProgress .dshm-time--end{text-align:right}
...
.dshm-pill--compact{flex:none;height:30px;border-radius:8px;font-size:11.5px;padding:0 10px}
...
.dshm-queueRow{position:relative;cursor:default;width:100%;color:var(--dsw-alias-label-primary);text-align:left;border:0;background:transparent;border-radius:8px;align-items:center;gap:10px;padding:6px 8px;font:inherit;display:flex}
...
.dshm-playerProgress .dshm-time{font-size:11.5px;font-variant-numeric:tabular-nums}
...
.dshm-slider::-webkit-slider-thumb{-webkit-appearance:none;appearance:none;width:13px;height:13px;background:var(--dsw-alias-label-primary);border-radius:50%;box-shadow:0 1px 3px rgba(0,0,0,.3);transform:scale(0);transition:transform .12s var(--ds-ease-in-out)}
.dshm-slider:hover:not(:disabled)::-webkit-slider-thumb,.dshm-slider:active::-webkit-slider-thumb{transform:scale(1)}
.dshm-slider::-moz-range-thumb{width:13px;height:13px;background:var(--dsw-alias-label-primary);border:0;border-radius:50%;transform:scale(0);transition:transform .12s var(--ds-ease-in-out)}
...
`;
    ...
    const ICONS = {
      ...
      queue: officialIcon(["IconQueueOutlineRegular", "IconQueueOutlineMedium"],
        14, h("svg", { width: 14, height: 14, viewBox: "0 0 16 16", fill: "none", stroke: "currentColor", strokeWidth: 1.3, strokeLinecap: "round" }, h("path", { d: "M2.5 4.5h11M2.5 8h7M2.5 11.5h7" }), h("path", { d: "M12.2 7.2v4.6" }), h("path", { d: "M13.6 9.6a1.4 1.4 0 1 1-2.8 0 1.4 1.4 0 0 1 2.8 0" }))),
      // DSH 图标集里没有星标（收藏），按你的规则补 macOS 原生风格的星形
    ...
    const ProgressBar = React.memo(function ProgressBar({ player, trackId, duration, time, playing, t, variant }) {
      ...
      // macOS Music 的全屏播放器：细轨在上，时间在下左右分列；底部条仍用一行内联
      if (variant === "stacked") {
        return h(React.Fragment, null,
          slider,
          h("div", { className: "dshm-timeRow" }, elapsedLabel, remainingLabel),
        );
      }
      return h(React.Fragment, null, elapsedLabel, slider, remainingLabel);
    });
    ...
    function MusicView() {
      ...
      useEffect(() => {
        if (state.pendingDelete >= 0) setPlayerOpen(false);
      }, [state.pendingDelete]);
      ...
      const fullPlayer = playerOpen && h("div", {
        ...
        h("div", { className: "dshm-playerBody" },
          h("div", { className: "dshm-playerStage" },
            ...
            h("div", { className: "dshm-playerProgress" },
              h(ProgressBar, {
                player,
            ...
            h("div", { className: "dshm-playerQueue" }, queueEntries.length === 0
              ...
              : queueEntries.map((entry) => {
                  ...
                  return h("div", {
                    ...
                    h(RowCover, { src: player.coverFor(entry.track) }),
                    h("span", { className: "dshm-queueText" },
                      h("span", { className: "dshm-queueTitle" }, view?.title ?? entry.track.title),
