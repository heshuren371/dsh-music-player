    ...
    function createPlayer() {
      ...
      const playIndex = (index) => {
        ...
        if (promise !== undefined) promise.catch(() => {
          // Decode/ load failures also surface through the "error" event; keep
          // one friendly message either way.
      ...
      return {
        ...
        toggle: () => {
          ...
          if (audio.paused) {
            const promise = audio.play();
            if (promise !== undefined) promise.catch(() => set({ playing: false, error: translate("error.unsupported") }));
          } else {
            audio.pause();
          }
        },
