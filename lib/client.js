...
.dshm-progress .dshm-slider{pointer-events:none;width:100%}
...
`;
    ...
    const ProgressBar = React.memo(function ProgressBar({ player, trackId, duration, time, playing, t, variant }) {
      ...
      /**
       * 指针位置 → 秒数。命中区自己有宽度就直接换算（点哪儿跳哪儿）；
       * jsdom / 零宽等量不到矩形的情况返回 null，退回按原生 value 处理。
       */
      const valueFromPointer = (event) => {
        const slider = sliderRef.current;
        if (slider === null) return null;
        // 量滑杆本体：它的盒子就是可见的那条轨道（容器还含时间文字）。
        const rect = slider.getBoundingClientRect();
        if (!(rect.width > 0) || !Number.isFinite(event?.clientX)) return null;
        const total = duration > 0 ? duration : 0;
        if (!(total > 0)) return null;
        const ratio = Math.max(0, Math.min(1, (event.clientX - rect.left) / rect.width));
        slider.value = String(ratio * total);
        return ratio * total;
      };
      const beginDrag = (event) => {
        if (event.button !== undefined && event.button !== 0) return;
        const slider = sliderRef.current;
        if (slider === null || slider.disabled) return;
        event.preventDefault?.();
        draggingRef.current = true;
        movedRef.current = false;
        setDragging(true);
        const fromPointer = valueFromPointer(event);
        pendingRef.current = fromPointer !== null ? fromPointer : Number(slider.value);
        paintRef.current(pendingRef.current);
        const track = trackRef.current;
        // Keep receiving moves once the pointer leaves the (thin) track.
        try { (track ?? slider).setPointerCapture(event.pointerId); } catch { /* jsdom/unsupported */ }
      };
      const moveDrag = (event) => {
        ...
        const fromPointer = valueFromPointer(event);
        if (fromPointer === null && !movedRef.current) return;
        movedRef.current = true;
        pendingRef.current = fromPointer !== null ? fromPointer : Number(slider.value);
        // 拖动期间只画界面，不去动音频：流式文件被反复 seek 会不断中断 Range
        // 请求（还会撞上插件的失败恢复/前跳逻辑），"快速拖动"就会卡死。
        paintRef.current(pendingRef.current);
      };
      const endDrag = (event) => {
        if (!draggingRef.current) return;
        ...
        if (track !== null) {
          try {
            if (typeof event?.pointerId === "number") track.releasePointerCapture(event.pointerId);
          } catch { /* capture already released */ }
        }
        // 松手才真正 seek（单击 = 立即快进到该位置）。
      ...
      const onInput = (event) => {
        ...
        // 拖动中：只记位置 + 画界面，不动音频（见 moveDrag 的说明）。
    ...
    function MusicView() {
      ...
      const fullPlayer = playerOpen && h("div", {
        ...
        className: "dshm-player" + (playerPhase === "closing" ? " dshm-player--closing" : ""),
