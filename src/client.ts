...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     */
    // 注：原有一个 `downgradedToLegacy` 只写不读（降级状态由 hostEntry 表达、
    // 由 onDowngrade() 上报），属重构残留，已删 —— 见 tsconfig 的 noUnusedLocals。
...
`;
    ...
    const ICONS = {
      ...
      chevronDown: officialIcon(["IconChevronDownOutlineRegular", "IconChevronDownOutlineMedium"],
        18, h("svg", { width: 18, height: 18, viewBox: "0 0 16 16", fill: "none", stroke: "currentColor", strokeWidth: 1.4, strokeLinecap: "round", strokeLinejoin: "round" }, h("path", { d: "M3.6 6.2 8 10.4l4.4-4.2" }))),
      ellipsis: officialIcon(["IconEllipsisOutlineRegular", "IconEllipsisOutlineMedium"],
        16, h("span", { className: "dshm-glyph" }, "⋯")),
    ...
    const set = (patch: Partial<PlayerState>) => {
      ...
      const schedulePoll = () => {
        ...
        pollTimer = setTimeout(async () => {
          pollTimer = null;
          if (disposed || !state.scanning) return;
          try {
      ...
      const restoreLastPlayed = () => {
        ...
        if (track.kind === "video") {
          ...
          void prepareVideo(track).then((url) => {
            ...
            cueSource(url);          // cue 成暂停态：不调 play，也就不会有 AbortError
          });
          return;
        }
       ...
       *
       * ⚠️ 不能用 `attachSource(url); audio.pause();` 来模拟「cue」：`attachSource` 内部
       * 调了 `audio.play()`，紧接着的同步 `pause()` 会让那个 promise 以 **AbortError**
       * 拒绝，而 catch 只放行 NotAllowedError ⇒ 每次打开页面都弹一条
      ...
      const startWasmVideo = async (track, stage) => {
        if (typeof document === "undefined" || stage === null || stage === undefined) return false;
        try {
          ...
          if (typeof loadPolyfill !== "function") return false;
          ...
          if (videoTrack === null || videoTrack === undefined) return false;
          ...
          void (async () => {
            for await (const frame of sink.canvases()) {
              if (token.aborted || playingId !== track.id) return;
              ...
              if (drift > 0.15) await new Promise((r) => setTimeout(r, Math.min(200, drift * 500)));
              if (drift < -0.4) continue;
              canvas.width = frame.canvas.width;
              canvas.height = frame.canvas.height;
              ctx.drawImage(frame.canvas, 0, 0);
            }
          })().catch(() => { /* 解到一半失败：画面停住，不影响音频 */ });
          return true;
        } catch {
          stopWasmVideo();
          return false;
      ...
      const recoverAt = (failedTrack, failedIndex, target, delay) => {
        setTimeout(() => {
          ...
          void (async () => {
            await ensureStreamBase();
            if (disposed || state.current !== failedIndex || playingId !== failedTrack.id) return;
      ...
      const guardVideoPicture = (track) => {
        setTimeout(() => {
          ...
          if (current === undefined || current.kind !== "video") return;
          if (Number(audio.videoWidth ?? 1) > 0 || audio.readyState < 2) return;
          if (mvEscalated.has(track.id)) return;
      ...
      audio.addEventListener("error", () => {
        ...
        if (failedSrc.includes("system-stream") && !sessionBaseHealed) {
          ...
          if (retryTrack !== undefined) {
            void (async () => {
              await ensureStreamBase();
              if (playingId !== retryTrack.id) return;
        ...
        if (failedTrack !== undefined && failedTrack.kind === "video" && !mvEscalated.has(failedTrack.id)) {
          ...
          void prepareVideo(failedTrack, "transcode").then((url) => {
            if (url === null || playingId !== failedTrack.id) return;
            attachSource(url);
          });
          return;
        }
          ...
          try {
            ...
            if (wasLoaded) {
              ...
              if (index >= 0) {
                ...
                void (async () => {
                  await ensureStreamBase();
                  if (disposed || playingId !== newId) return;
                  audio.src = streamUrl(track);
                  audio.addEventListener("loadedmetadata", () => {
    ...
    const Marquee = React.memo(function Marquee({ text, className, title }) {
      ...
      useEffect(() => {
        ...
        if (host === null || sample === null) return undefined;
        const sync = () => {
      ...
      return h("div", {
        ...
        h("div", { className: "dshm-marqueeInner" },
          h("span", { className: "dshm-marqueeText", ref: sampleRef }, text),
    ...
    const RowCover = React.memo(function RowCover({ src, playing, paused }) {
      ...
      if (!playing) return art;
      return h("span", { className: "dshm-coverWrap" },
        art,
        h("span", { className: "dshm-coverBars", "aria-hidden": "true" }, h("i"), h("i"), h("i")),
        h("span", { className: "dshm-coverHover", "aria-hidden": "true" }, paused ? ICONS.play : ICONS.pause),
      );
    });
        ...
        h("table", { className: "dshm-table" },
          ...
          h("tbody", null, rows.map(({ track, index }) => {
            ...
            const rowElement = h("tr", {
              ...
              h("td", { className: "dshm-cellTitle" }, item.title, track.kind === "video" ? h("span", { className: "dshm-mvTag" }, "MV") : null),
    ...
    function MusicView() {
      ...
      useEffect(() => () => {
        ...
        if (media === null || media === undefined || typeof media.nodeType !== "number") return;
        if (park !== null && park !== undefined && media.parentElement !== park) park.appendChild(media);
      }, []);
      useEffect(() => {
        ...
        if (target === null || target === undefined || media.parentElement === target) return;
      ...
      useEffect(() => {
        ...
        if (media.poster !== poster) media.poster = poster;
      ...
      const queueEntries = (() => {
        ...
        if (at <= 0) return rows.slice(0, 20);
      ...
      const fullPlayer = playerOpen && h("div", {
        ...
        h("div", { className: "dshm-playerScrim" }),
        h("div", { className: "dshm-playerTop" },
          withTipWrapped(h("button", {
            type: "button",
          ...
          h("div", { className: "dshm-playerVolume" },
        ...
        h("div", { className: "dshm-playerBody" },
          h("div", { className: "dshm-playerStage" },
            ...
            isVideo
              ...
              ? h("div", {
                  ...
                  mvState !== null && mvState !== undefined && mvState.state !== "ready"
                    ? h("div", { className: "dshm-mvPrep" },
                        h("span", { className: "dshm-mvPrepText" }, mvState.state === "failed"
                          ? t("mv.failed")
                          : mvState.mode === "transcode" ? t("mv.transcoding") : t("mv.remuxing")),
                        mvState.state !== "failed" && h("span", { className: "dshm-mvPrepBar" },
                          h("i", { style: { width: Math.round((mvState.progress ?? 0) * 100) + "%" } })),
                        mvState.state !== "failed" && h("span", { className: "dshm-mvPrepPct" }, Math.round((mvState.progress ?? 0) * 100) + "%"),
                        mvState.state === "failed" && mvState.error === "ffmpeg-not-found"
            ...
            h("div", { className: "dshm-playerMeta" },
              h("div", { className: "dshm-playerMetaText" },
              ...
              withTipWrapped(h("button", {
                ...
                onClick: (event) => {
                  event.stopPropagation();
                  if (currentTrack !== undefined) toggleFavorite(currentTrack.id);
                },
            ...
            h("div", { className: "dshm-playerTransport" },
              h("span", { className: "dshm-playerTransportSide" }),
              h("div", { className: "dshm-playerTransportCenter" },
                transportButton(ICONS.prev, t("action.prev"), () => player.prev()),
                transportButton(state.playing ? ICONS.pauseBig : ICONS.playBig, state.playing ? t("action.pause") : t("action.play"), () => player.toggle(), " dshm-playerTransportBtn--play"),
                transportButton(ICONS.next, t("action.next"), () => player.next()),
              ),
              h("div", { className: "dshm-playerTransportSide dshm-playerTransportSide--end" },
                transportButton(
                  state.mode === "loop" ? ICONS.repeat : ICONS.repeatOne,
                  state.mode === "loop" ? t("mode.loop") : t("mode.one"),
                  () => player.toggleMode(),
                  state.mode === "one" ? " dshm-playerTransportBtn--on" : "",
                ),
              ),
            ),
          ),
          h("div", { className: "dshm-playerRight" },
            h("div", { className: "dshm-playerPills" },
              withTipWrapped(h("button", {
                type: "button",
                className: "dshm-pill",
              ...
              }, ICONS.refresh, t("action.refresh")), t("action.refresh")),
            ),
            h("div", { className: "dshm-playerQueueHead" },
              h("span", { className: "dshm-playerQueueTitle" }, t("player.queue")),
              h("span", { className: "dshm-playerQueueCount" }, t("stats")(queueEntries.length)),
            ),
            h("div", { className: "dshm-playerQueue" }, queueEntries.length === 0
              ...
              : queueEntries.map((entry) => {
                  ...
                  return h("div", {
                    ...
                    h("span", { className: "dshm-queueText" },
                      h("span", { className: "dshm-queueTitle" },
                        entry.track.kind === "video" ? h("span", { className: "dshm-mvTag" }, "MV") : null,
                        view?.title ?? entry.track.title),
                      h("span", { className: "dshm-queueArtist" }, subtitle.length > 0 ? subtitle : entry.track.name),
                    ),
                    favorited && h("span", { className: "dshm-queueFav" }, ICONS.starFill),
                    withTipWrapped(h("button", {
                      type: "button",
                    ...
                    }, ICONS.ellipsis), t("player.more")),
                    queueMenu === entry.track.id && h("div", {
      ...
      const bar = h("div", { className: "dshm-bar" },
        h("div", { className: "dshm-nowPlaying" },
          ...
          h("div", { className: "dshm-nowText" },
            ...
            currentTrack?.kind === "video" ? h("span", { className: "dshm-mvTag" }, "MV") : null,
            h(Marquee, {
      ...
      return h("div", { className: "dshm-root", ref: rootRef },
        ...
        h("div", { className: "dshm-mvPark", ref: mvParkRef, "aria-hidden": "true" }),
        header,
        dirEditor,
        completeBar,
        confirmBar,
        state.hostEntry === "legacy" && h("div", { className: "dshm-notice" }, t("error.legacyEntry")),
