...
check('playing before teardown', audio.playCount === 1, 'playCount=' + audio.playCount);
// load-time decode error -> schedules an auto-skip 600ms later
audio.currentTime = 0;
...
haltEntry.d();
await settle(900);
check('teardown cancels the pending auto-skip (no restart)', audio.playCount === 1, 'playCount=' + audio.playCount);
check('teardown detached the stream', audio.src === '', 'src=' + JSON.stringify(audio.src));
